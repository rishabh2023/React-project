...
export default function MonClick() {
    ...
    const getdata1 = async() => {
        ...
        await axios.get(`/events/get_event/${id}/?date=${m2}`).then((resp) => {
    ...
    return (
        ...
        >
            ...
            {slots.length === 0 ? <p className='text-sm font-semibold text-center text-red-500 mt-1'>Opps! No Slots Available</p> : slots.map((value,idx) => (
                <div className="ml-1 text-xs inline-flex items-center font-bold leading-sm uppercase px-2 py-1 bg-green-200 text-green-700 rounded-full m-2 justify-evenly"
                    onClick={() => {
                        setserviceid(id)
                        seteventid(value.id)
                        setdate(m2)
