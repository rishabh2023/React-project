...
export default function Expertcard() {
    const id  = JSON.parse(localStorage.getItem('profile_id'))
    ...
    const getExpert = async () => {
        await axios.get(`/user/expert/${id}/`).then((resp) => {
            ...
            setimgsrc(resp.data.expert_profile?.personal_detail[0]?.profile_img)
    ...
    return (
        <>
            ...
            <div className="lg:w-1/4 md:w-1/2 p-3  border-2 shadow-2xl text-center border-r-20 rounded-3xl w-full ml-10 mr-10 mb-10 mt-8 bg-white">
                <a className="block h-48 rounded overflow-y">
                    <img alt="Expert" className="object-cover object-center w-40 h-40 rounded-full mx-auto " src={imgsrc === ''?`https://ui-avatars.com/api/?name=${Name}`:imgsrc} />
                </a>
                <div className=" container object-center -mt-2">
                    <h3 className="text-gray-900 title-font text-lg font-bold capitalize" onClick={()=>{history.push(`/profile/${id}`)}}>{Name}</h3>
