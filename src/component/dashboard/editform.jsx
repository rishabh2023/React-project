...
export default function EditForm() {
  ...
  const getdata = async() =>{
      await axiosInstance.get(`/user/service_detail/${id}/`).then((resp) => {
          console.log(resp.data)
          sessionStorage.setItem('service_id',resp.data.id)
          setTitle(resp.data.service_name)
            const mprice = Number(resp.data.price)
            
          setPrice(mprice)
          setDis(resp.data.description)
  ...
  useEffect(() => {
    setValue('service_name',title)
    setValue('price',price)
    setValue('description',dis)
    setValue('category',category)

    
    getdata();

}, [title,price,dis,category])
  ...
  const onSubmit = async (data, e) => {
    ...
    const pid = jwt_decode(JSON.parse(localStorage.getItem('access_token')));
    ...
    await axiosInstance.patch(`/user/service_update/${id}/`, data_payload).then((res) => {
  ...
  return (
    <>
      ...
      <div className="">
        ...
        <form className="m-3" onSubmit={handleSubmit(onSubmit, onError)}>
          ...
          <div className="md:flex md:items-center place-content-center mb-6">
            <div className="md:w-2/3">
              ...
              >
                 ...
                 <option value={Number(value.id)}  selected={value.id === category?true:false}> {value.name}</option>
                 ))
          ...
          <div className="md:flex md:items-center place-content-center mb-6">
            <div className="md:w-2/3">
              ...
              <textarea className="block appearance-none w-full  shadow-md bg-gray-100 border-2 border-blue-300 hover:border-blue-700 px-4 py-2 pr-8 rounded  leading-tight focus:bg-white focus:outline-none focus:shadow-outline"
                name="Brief Description of service" 
          ...
          <div className="md:flex md:items-center  place-content-center mb-6">
            <div className="md:w-2/3">
              ...
              > Submit</button>}
                ...
                <button
                className=" float-left mt-2 mb-2 w-auto text-justify  shadow-md text-white font-semibold bg-blue-700 border-2   px-4 py-2 pr-8 rounded-xl  leading-tight focus:outline-none disabled:bg-gray-400 disabled:cursor-not-allowed mx-auto "
                onClick ={()=>{
                  
                  history.push(`/dashboard/service/create/timeslots/${id}`)
                    setclick1('clicked')
                }}
                type='submit'
